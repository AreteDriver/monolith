...
class ContinuityChecker(BaseChecker):
    ...
    def _check_c3_state_gaps(self) -> list[Anomaly]:
        ...
        for row in objects_with_snapshots:
            ...
            if isinstance(new_state_raw, dict):
                new_state = new_state_raw.get("variant", str(new_state_raw))
            else:
                new_state = str(new_state_raw)
            if isinstance(old_state_raw, dict):
                old_state = old_state_raw.get("variant", str(old_state_raw))
            else:
                old_state = str(old_state_raw)
